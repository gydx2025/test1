...
FAST_FAIL_CONFIG = {
    'enabled': True,                    # 是否启用快速失败
    'request_timeout': 10,              # 单个请求超时（秒）
    'max_retries': 2,                   # 最大重试次数
    'retry_delays': [1, 2],             # 重试延迟（秒）
    'skip_after_failures': 3,           # 连续失败N次后跳过
}
...
SOURCE_SELECTION_CONFIG = {
    'enabled': True,                    # 是否启用智能源选择
    'min_success_rate': 0.05,           # 最小成功率（低于此值的源放弃）
    'max_sources_per_stock': 3,         # 每只股票最多尝试的源数
}
