...
**依赖说明**：
- 核心依赖：pandas, requests, beautifulsoup4 等
- **图形界面依赖**：PyQt5 >= 5.15.0, PyQt5-sip >= 12.11.0

如果只需要命令行版本，可跳过 PyQt5 安装。
...
```bash
# 方式1：通过 requirements.txt 一键安装（推荐）
pip install -r requirements.txt

# 方式2：单独安装 PyQt5
pip install PyQt5>=5.15.0 PyQt5-sip>=12.11.0
```

**兼容性说明**：
- Python 3.7+：支持 PyQt5 5.15.x
- Windows/Linux/macOS：均支持
- 无显示器环境：需要配置虚拟显示（如 Xvfb）
...
# 后台启动（Linux/Mac）
nohup python3 astock_real_estate_collector_gui.py > gui.log 2>&1 &
```
...
- 复选项：
  ...
  - [x] 完整性检查（默认启用）
    - 确保必填字段不为空
    - 检查数据逻辑一致性
- 建议：生产环境全部启用，测试时可禁用以加快速度

**2.3 股票过滤** 🏢
- **按代码筛选**：
  - 单个：600000
  - 范围：600000-600999
  - 多个：600000,000001,000002
- **按名称筛选**：
  - 关键词匹配（如：银行、地产）
  - 支持正则表达式
- **按简称筛选**：
...
3. **实时日志窗口**
   - 滚动显示最新日志
   - 颜色标识：INFO（黑色）、WARNING（橙色）、ERROR（红色）
   - 支持搜索和过滤
   - 可导出日志到文件
...
1. **表格预览**
   - 显示已采集的数据
   - 列：股票代码、名称、2023资产、2024资产、变化、行业、市场
   - 支持排序：点击列头排序
   - 支持筛选：顶部搜索框
...
3. **文件命名规则**
   - 预设模板：
     - A股非经营性房地产资产_{日期}_{时间}（默认）
     - 自定义前缀 + 时间戳
     - 完全自定义
   - 示例：A股非经营性房地产资产_20241216_153045.xlsx
...
5. **Excel 特殊选项**（仅 Excel 格式）
   - [ ] 分表导出（基础信息、行业、财务、统计、元数据）
   - [ ] 添加数据透视表
   - [ ] 冻结首行
   - [ ] 自动调整列宽
   - [ ] 添加筛选器
...
1. **依赖检查**：
   - 首次运行前确保已安装 PyQt5
   - 使用 `run_gui.sh` 会自动检查并安装依赖

2. **资源占用**：
   - GUI 比 CLI 占用更多内存（约 +100-200MB）
   - 推荐系统内存 ≥ 4GB
